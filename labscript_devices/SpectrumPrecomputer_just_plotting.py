...
# Added for the plus_printing version to print the final waveform
...
FIGURES_FOLDER = "C:\\labscript-suite\\labscript-devices\\labscript_devices\\SpectrumWaveformFigures\\"
...
class SpectrumPrecomputer:
    ...
    def compute_waveform(self):
        ...
        # Sort the groups in time order (just in case)
        self.waveform_groups = sorted(self.waveform_groups, key=lambda k: k.time)

        # We save the groups output at the beginning, before we add dummy data
        pickle.dump(self.waveform_groups, open(self.output_groups_name, "wb")) # THOUGHT: could pickle load here (1/3)
        ...
        for seg_idx, group in enumerate(self.waveform_groups):
            ...
            if group.waveforms != "dummy":
                ...
                for wvf in group.waveforms:
                    ...
                    #######################################################################
                    # Calculate start time, end time, and duration of waveform in seconds #
                    #######################################################################
                    ...
                    # a temporary array used to aggregate pulse data to build the pulse_data object
                    pulse_data_temp = np.array([])

                    #################################
                    # Populate pulse_data with data #
                    #################################
                    ...
                    if hash(wvf) in self.pulse_dictionary.keys() and not wvf.has_mask:
                        ...
                        ###############################
                        ##  Saving waveform figures  ##
                        ###############################
                        # Addition by JJ on 2/24/2025 #
                        ###############################

                        # TODO: Make saving based off of runmanager variable or other variable

                        fig, axs = plt.subplots(2, 1, sharex=True)
                        axt, axb = axs

                        axt.plot(t, pulse_data)
                        axt.title(f"Waveform for port {wvf.port} at time {wvf.time} (remove_self_interaction = {wvf.remove_self_interaction})")
                        axb.title(f"Modulation waveform")
                        axb.plot(t, modulation_waveform)
                        axt.ylabel("Amplitude")
                        axb.ylabel("Amplitude")
                        axb.xlabel("Time (s)")

                        fig.savefig(FIGURES_FOLDER + f"{self.numpy_name}_port_{wvf.port}_time_{wvf.time}.png")

                        plt.close(fig)

                        # todo save to FIGURES_FOLDER

                        ## End of addition by JJ on 2/24/2025 ##
        ...
        np.save(self.output_name, segments, allow_pickle=True) # THOUGHT: could pickle load here instead of precompute (2/3)
    ...
    def transition_to_buffered(self, device_name):
        """
        In SpectrumPrecomputer, we run transition_to_buffered as if we were
        actually running it, but instead save the computed Spectrum waveforms
        to a file.

        There are three files to keep track of:
            - "spectrum_{device_name}.npy" (self.output_name):
              - Contains dictionary of digitized waveform segments 
                that will be played by the card, including carrier waves,
                modulation, and masking
            - "spectrum_{device_name}_groups.npy" (self.numpy_groups_name):
            - "spectrum_{device_name}_dict.npy" (self.device_dict_name):
        """

        # yes, the code here is confusing. It might be helpful to do some cleanup
...
# JJ 2/24/2025: What is this function for?
    ...
    while True:
        ...
        while all_shots:
            ...
            try:
                ...
                # compute if haven't we've already generated a file for it
